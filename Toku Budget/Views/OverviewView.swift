...
struct OverviewView: View {
    ...
    init(window: DateWindow, mode: DateRangeMode) {
            ...
            // Budgets (still monthly; use the month containing window.start)
    ...
    /// % change vs last month; nil if not enough data or last month is 0.
    ...
    /// Use the largest single budget amount as the chart's Y max (optional).
    private var spendYMaxFromBudgets: Double? {
        budgets.map { ($0.amount ?? 0).doubleValue }.max()
    }

    /// Cumulative net sparkline for the Net Balance card
...
struct NetBalanceCard: View {
    let amount: Decimal
    let pctChange: Double?        // nil => hide the “vs last month” line
...
// Mini budgets rings (unchanged logic)
...
// Spending bars with optional Y max
struct SpendBars: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            // Compact, unclipped weekday marks
            .chartXAxis {
                AxisMarks(values: .stride(by: .day, count: 2)) { value in
                    AxisGridLine(); AxisTick()
                    if let d = value.as(Date.self) {
                        AxisValueLabel {
                            Text(d, format: .dateTime.weekday(.narrow)) // M T W …
                                .font(.caption2)
                        }
                    }
                }
            }
            // Cap Y to budget max if provided
            .chartYScale(domain: {
